...
Permisos que gobiernan el módulo "Admin. Cajas" (`/adminCash/adminCash`, accesible desde el sidebar Facturar → "Crear y asignar cajas", fuera del propio POS), el modal "Detalle de Cierre" del POS (`#dialog_cash_closing`, mostrado por "(F12) Abrir/Cerrar Caja" cuando la caja ya está abierta) y Reportes → "Cierres de Caja" (`/reports/cashReport`, aprobación de cierres pendientes). A diferencia del resto del catálogo, **7 de estos 9 ids no aparecen en el JSON de `getRolePermissionById`** (523 entradas) **— solo en el DOM servido de "Roles y permisos"** (526 filas reales): son permisos más nuevos que el propio catálogo AJAX de "Notas" todavía no indexa. El nombre/id/Nota se leyeron directamente del DOM real (`tr.innerText` + `id` del checkbox), no del JSON, y cada uno se confirmó igual por su efecto real en vivo, nunca solo por el texto.

| Permiso (nombre visible) | id | slug | Comportamiento real confirmado |
|---|---|---|---|
| Agregar caja | 41 | `addcash` | Controla el botón "Crear caja" (`button[onclick="addCash()"]`) + su input "Nombre de caja a agregar" en Admin. Cajas. Con el permiso desactivado ambos desaparecen; la página en sí sigue siendo accesible (sin el bug de auto-bloqueo que sí tiene "Admin roles" del POS). |
| Ver reporte de movimientos de caja | 83 | `seecashmovementreport` | Controla el ítem "(F8) Historial Mov. de Caja" del menú "Caja" del encabezado del POS (`#menu_cash`) y el acceso directo a `/cash_movement/movements` (redirige a `/help/unauthorized` sin el permiso). **No** controla el ítem vecino "(F9) Movimientos de caja", que permanece igual con el permiso activado o desactivado. |
| Ocultar total general en cierre de caja | 558 | *(no en JSON)* | Controla la píldora "Total general: $X" (`#total_cash_entries`) del encabezado del modal "Detalle de Cierre". Activado la oculta; desactivado la muestra. No afecta ningún otro total/subtotal del modal. |
| Ocultar opción de impresión en cierre de caja | 571 | `hidePrintCashClosing` | **No** agrega ni quita ningún botón visible dentro del modal "Detalle de Cierre" — controla si, tras confirmar "Cerrar Caja", se abre automáticamente el popup de impresión del cierre (confirmado contando eventos `popup` reales: 0 con el permiso activado, 1 con el permiso desactivado). |
| Ver resumen de utilidad de caja en el detalle de cierre de caja | 619 | `seecashprofitsummarycashclosuredetails` | Controla el tile "Utilidad" (fila de KPIs Ventas Totales/Descuento/Impuestos/Utilidad) del modal "Detalle de Cierre". Activado lo muestra; desactivado lo oculta por completo. |
| Restringir cierre de caja con órdenes de taller sin facturar | 575 | *(no en JSON)* | Con el permiso activo y al menos una orden de Taller sin facturar (`PosTaller.obtenerIdsOrdenesVisibles()`, el ambiente compartido siempre tiene varias acumuladas), "Cerrar Caja" queda bloqueado: el modal "Detalle de Cierre" permanece abierto tras confirmar el SweetAlert de "¿Está seguro...?". Desactivado, el cierre se completa normal. Ver el bug de sistema documentado abajo (el propio SweetAlert de bloqueo). |
| Validar cierre de caja | 673 | *(no en JSON)* | Activo, simplifica radicalmente el modal "Detalle de Cierre": quedan solo 2 tabs de moneda ("$ - General"/"$ - Facturas", de 8 normalmente) y desaparecen la fila de KPIs (Ventas Totales/Descuento/Impuestos/Utilidad), "Resumen de cierre", "Mostrar Reporte Avanzado" y los campos "Total"/"Diferencia" de "Datos de Cierre" — el cajero solo puede ingresar un conteo a ciegas, sin ver ningún monto esperado. El cierre generado queda "pendiente de aprobación" (fila `pending-tr` en `/reports/cashReport`) hasta que alguien con CAMBIAR_ESTADO_CIERRES_CAJA (674) lo apruebe/rechace. Sin el permiso, el modal muestra todo el detalle normal y el cierre queda final de inmediato (sin cola de aprobación). |
| Abrir caja aún pendiente de aprobación | 567 | `seeopenboxstillpendingapproval` | Solo tiene efecto observable cuando VALIDAR_CIERRE_CAJA (673) está activo: sin 673, ningún cierre queda "pendiente" y no hay nada que bloquear. Con 673 activo y un cierre propio sin aprobar: 567 activo permite abrir una caja nueva de todas formas; 567 desactivado bloquea la apertura (modal "Abrir Caja" permanece visible tras intentarlo) hasta que el cierre anterior sea aprobado. |
| Cambiar estado de cierres de caja | 674 | *(no en JSON; permiso no pedido originalmente, hallazgo de esta investigación)* | Controla el botón "Validar" (ícono de reloj, `title="Validar"`, `button.btn-cash-state`) de un cierre pendiente en Reportes → "Cierres de Caja". Activado, el botón existe y, al usarlo, permite aceptar/rechazar el cierre (`changeStateCashClosure`, único endpoint real detrás de las 3 funciones de esa página: `confirm()`, `confirm_allow_open_box()`, `change_state()`). Desactivado, el botón no existe para ningún cierre pendiente. |

**Mecanismo alternativo para togglear permisos sin salir del POS**: además de `/roleAdmin/roleAdmin` (`irARolesYPermisos()`/`establecerPermiso()`), existe el modal **"Permisos del POS"** (ítem del menú de tres puntos del encabezado del POS, el mismo que ya controla el permiso "Admin roles") — organiza el mismo catálogo de 71 permisos "que impactan directamente el POS" en 5 secciones (Acceso y navegación POS, Productos y líneas, Facturación/impresión/pagos, **Caja**, Cliente y crédito) como una matriz permiso×rol (`.pos-permission-modal`, checkboxes reales `input[data-permission-id][data-role-id]` — confirmado en vivo que `data-role-id="1"` es exactamente "Administrador nivel 1", primera columna). Escribe el mismo permiso real (mismo endpoint `SetPermissionToRole`/`deletePermissionRole`) sin navegar fuera del POS — `PosPermisos.abrirModalPermisosDelPos()` / `expandirSeccionCajaEnModalPos()` / `establecerPermisoEnModalPos()` / `cerrarModalPermisosDelPos()`. Usado únicamente en el escenario de Validar cierre/Abrir pendiente/Cambiar estado (el más sensible a la latencia de navegación, por requerir generar y aprobar un cierre real de por medio); el resto de la suite sigue con `irARolesYPermisos()`, ya extensamente probado.

**Bug de sistema confirmado en vivo (i18n, Restringir cierre con órdenes de taller)**: cuando el cierre queda bloqueado por este permiso, el SweetAlert de confirmación de siempre ("¿Está seguro(a) de que desea cerrar esta caja?") agrega el texto real **sin traducir "Not valid!"** mezclado con el resto del mensaje en español — un string en inglés que se filtró al flujo de validación sin pasar por el sistema de traducciones de la app.

**Bug de sistema confirmado en vivo (HTTP 500 intermitente, más frecuente en 673/571 que en el resto del catálogo)**: el mismo bug ya documentado arriba (`SetPermissionToRole` → `FatalErrorException`) es notablemente más frecuente togleando el id 673 — confirmado en vivo que, a diferencia de otros ids donde el valor "igual queda guardado pese al 500", con 673 el valor a veces **NO** persiste tras un 500 (`esperarPermiso()` agotando 40s sin ver el cambio) y a veces sí. Cualquier automatización sobre este id debe verificar con `esperarPermiso()` tras cada `establecerPermiso()` y estar preparada para reintentar, nunca asumir éxito tras un único intento.

**Bug de sistema confirmado en vivo (`validate_cash()` puede no abrir su propio diálogo bajo carga)**: el botón "Validar" de un cierre pendiente dispara `validate_cash(cash_id, this)`, que hace un XHR **síncrono** (`async:false`) a `validateCash` antes de mostrar `#dialog_validate_cash` — confirmado en vivo (2/2) que, bajo carga del ambiente compartido, ese XHR puede no completar nunca (o fallar en silencio), dejando el diálogo sin aparecer pese a que el click sí llegó al botón real. `PosPermisos.aprobarCierrePendiente()` intenta primero el flujo de UI real y, si falla, cae a `aprobarCierrePendienteViaApiDirecta()` (llamada directa a `changeStateCashClosure`, mismo endpoint real, confirmado en vivo con `option=1` → respuesta `"1"`) como red de seguridad — mismo criterio que `establecerPermisoViaApiDirecta()` para "Admin roles".
