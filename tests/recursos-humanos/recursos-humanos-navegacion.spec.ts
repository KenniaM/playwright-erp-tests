...
import { RecursosHumanosPage, SUBMODULOS_RECURSOS_HUMANOS, TIMEOUTS } from './recursos-humanos.page';
import { espiarErroresJS } from '../facturar/pos/pos.utils';

for (const submodulo of SUBMODULOS_RECURSOS_HUMANOS) {
  test(`Cargar el submódulo "${submodulo.nombre}" del módulo Recursos Humanos`, async ({ page }) => {
    test.setTimeout(TIMEOUTS.TEST);
    const recursosHumanos = new RecursosHumanosPage(page);
    const erroresJS = espiarErroresJS(page);

    await test.step(`Navegar a "${submodulo.nombre}"`, async () => {
      await recursosHumanos.irA(submodulo.url);
    });
    ...
    await test.step('Validar que no ocurrieron errores de JavaScript', async () => {
