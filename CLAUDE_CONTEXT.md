...
- Cualquier lectura de "el registro más reciente" en un listado compartido (ej. `abrirPrimeraFacturaDelListado()` en Histórico de Ventas) puede leer un registro creado por OTRA actividad del ambiente compartido entre la acción del test y la lectura, no necesariamente el propio — confirmado en vivo repetidas veces (ver `metodo_pago_mismatch_bug.md` en memoria y hallazgos de sesiones de Restaurante). Si un total/forma de pago no coincide pero el flujo de facturación en sí no mostró ningún error, sospechar esto antes que un bug de cálculo — documentarlo como hallazgo de ambiente, no debilitar la aserción ni intentar "arreglarlo" filtrando por heurísticas frágiles.
...
**`.evaluate()` para clicks "nativos" siempre necesita su propio `timeout`.** Varios botones de este repo (widgets MDL con animación continua, ej. `abrirMenuMesa()`) usan `locator(...).evaluate((el) => el.click())` en vez de `.click()` real para evitar el mismo problema de accionabilidad de Playwright. Pero `Page.evaluate()` NO acepta `timeout` — `Locator.evaluate(fn, arg, {timeout})` sí. Sin ese tercer argumento, si el elemento no está listo en el primer intento de un bucle de reintentos, el `.evaluate()` puede quedar esperando indefinidamente (este proyecto no configura `actionTimeout` global) y NUNCA llega a los siguientes intentos — confirmado en vivo (un test se agotó a los 180s exactos en una sola línea). Cualquier `.evaluate()` de este estilo dentro de un bucle de reintentos debe llevar `{ timeout: N }` + `.catch(() => {})`, igual que un `.click({ timeout })` normal.

**Ventanas emergentes de impresión (`window.open`) deben cerrarse explícitamente tras confirmarlas.** `esperarVentanaImpresion()` (`pos.utils.ts`) solo esperaba el evento `popup` y lo dejaba abierto. Confirmado en vivo (3/3): si un flujo dispara DOS impresiones de este estilo en sucesión rápida (ej. Comanda "NUEVOS" seguido de "TODOS" sobre la misma mesa), la segunda puede nunca disparar el evento `popup` — el `POST` real sí viaja y responde 200, pero el navegador bloquea el `window.open()` mientras la ventana anterior sigue abierta (ambos disparados por clicks sintéticos vía `.evaluate()`, no un gesto de usuario real, que los navegadores tratan con más sospecha). La función ahora cierra la ventana propia (`await popup.close()`) tras confirmarla — cualquier flujo nuevo que dispare 2+ impresiones seguidas debe apoyarse en esta función, no reimplementar `waitForEvent('popup')` suelto.

**Antes de concluir que un fallo es un bug real, re-ejecutar el escenario aislado.** Bajo el `fullyParallel` de este repo, correr una suite completa con varios workers contra el ambiente compartido puede producir una tasa de fallos muy alta (hasta 30-60% observado en sesiones reales) que no refleja bugs de código — confirmado repetidas veces re-corriendo exactamente los mismos escenarios fallidos con `--workers=1` (usando `archivo.spec.ts:<línea>` para apuntar a tests puntuales) y viéndolos pasar limpio. Solo tratar un fallo como bug real de automatización/sistema cuando se reproduce de forma consistente TAMBIÉN en aislamiento (idealmente 2-3 veces) — un fallo que solo aparece bajo carga paralela y desaparece aislado es contención del ambiente, no código roto, y no debe "corregirse" debilitando una aserción ni añadiendo reintentos genéricos.
